...
# ==========================================
# 📊 VERİ YÜKLEME VE SÜTUN DÜZELTME
# ==========================================
...
def verileri_yukle():
    ...
    if os.path.exists(ebrd_yolu):
        try:
            ...
            # Sütun kayma düzeltmesi: Eğer sütun isimleri yanlışsa doğru indeksleri atıyoruz
            df.columns = ["Kurum", "Ülke", "İlan Tipi", "İhale/Proje Adı", "Tarih / Son Başvuru", "Link"]
            df.fillna("Belirtilmemiş", inplace=True)
            return df
        except:
            pass
            
    # Yedek Veri (Sütunlar garantili)
    return pd.DataFrame({
        "Kurum": ["EBRD"]*8,
        "Ülke": ["Romania", "Serbia", "Poland", "Croatia", "Bosnia and Herzegovina", "Ukraine", "Albania", "Hungary"],
        "İlan Tipi": ["General Procurement Notice"]*8,
        "İhale/Proje Adı": [
        ...
        "Link": ["https://ecepp.ebrd.com/delta/viewNotice.html?displayNoticeId=12345"] * 8
    })
...
filtrelenmis_df = df[~df["İhale/Proje Adı"].str.lower().str.contains('|'.join(haric_tutulacaklar), na=False)]

st.success(f"Sistem Aktif: Onaylı {len(filtrelenmis_df)} Üst Yapı Projesi Listeleniyor.")
